...
本文是 HRMS 设计前的调研输入，不是最终设计稿。正式设计稿需要在一期范围确认后再编写。
...
如果确认方案 B，下一步输出正式设计文档，内容包括模块边界、表结构草案、接口清单、权限点、页面清单、审计要求、测试策略和分期计划。
...
这样做的目的不是把 HRMS 立即写进系统接口，而是让 Jenkins、接口管理页和人工巡检能持续看到“下一步企业支撑能力”的设计责任，避免 HRMS 停留在一次性调研文档里。
