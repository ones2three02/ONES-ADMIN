...
- 员工资料附件已选择复用 `sys_file` 和 `HR_EMPLOYEE_DOCUMENT` 业务归属接入员工档案，并在 v0.0.91 通过 `hr_employee_document` 承载资料分类和有效期；v0.0.92 先把到期风险接入概览和预警接口，v0.0.93 已补齐资料预警前端运营页，长期归档和合规保留策略继续后置。
