...
- v0.0.77 起，员工列表和花名册导出默认脱敏手机号、邮箱和证件号；员工详情保留 HR 编辑所需完整联系方式，后续再按字段级权限决定是否对详情继续脱敏。
- 前端编辑员工时必须重新拉取详情，不得直接把列表脱敏值作为编辑表单初值提交，避免脱敏展示值污染主数据。
