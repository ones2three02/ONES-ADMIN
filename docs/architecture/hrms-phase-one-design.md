...
- 合同附件前端上传/预览、文件访问策略、更深入的 HRMS 报表分析和更完整的员工详情页仍按后续阶段推进。
...
- 合同附件前端是否直接接入系统文件管理页，还是先在合同抽屉内提供轻量上传/预览入口。
