...
- `v0.0.147` 已治理 HRMS 资料预警、岗位、职级和花名册导入等业务页表格稳定高度，并同步系统业务表格自动高度清单归零，本版本不新增数据库迁移。
- `v0.0.148` 已关闭正式运行默认初始化演示管理员的风险，并补齐账号/IP 登录限流、数据库失败次数原子更新、受控公开端点和 Flyway 安全默认值，本版本不新增数据库迁移。
- `v0.0.149` 已建立飞书 OAuth 与本地账号预绑定闭环，外部平台只证明身份，HRMS 角色、数据范围和字段权限仍由 ONES-ADMIN 决定。
