...
- HRMS 已完成一期企业级设计稿，明确 `hr` 业务域、`hr_` 表前缀、员工主数据、任职记录、生命周期事件、合同、花名册导入导出、权限码、接口清单、审计和前端页面边界。
...
- HRMS 当前仍处于设计阶段，待确认后需要新增 Flyway 增量脚本、后端 `com.ones.admin.hr` 业务包和 Vben 风格前端页面。
