...
- `main`：稳定版本、正式环境部署来源。
- Jenkins 先接 `develop` 做验证门禁，根目录 `Jenkinsfile` 已在 v0.0.129 固化当前阶段的测试、类型检查、构建和仓库扫描，在 v0.0.130 补齐版本一致性门禁，在 v0.0.131 统一为 `scripts/ci/verify.sh` 阶段入口，在 v0.0.132 增加构建元数据归档，在 v0.0.133 增加机器可读验证摘要归档，并在 v0.0.134 增加接口治理报告归档，不包含部署动作：
