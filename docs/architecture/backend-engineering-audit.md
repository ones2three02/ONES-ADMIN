...
- 敏感字段治理已在 v0.0.77 完成第一阶段闭环：新增通用脱敏工具，HRMS 员工列表和花名册导出默认脱敏手机号、邮箱和证件号，前端编辑抽屉打开时重新拉取详情，避免列表脱敏值被误写回库。v0.0.78 已继续补齐 `hr:employee:sensitive:view` 字段级查看权限，员工详情和写接口响应按权限决定联系方式是否完整展示，无权限编辑时后端保留原手机号和邮箱。
...
- 新增 HRMS 敏感字段脱敏基础治理：
  ...
  - 本轮对标 Smart Admin、RuoYi-Vue-Plus、JeecgBoot 的数据脱敏基础能力；ONES-ADMIN 当前先采用轻量工具层和字段权限落地，后续再演进为注解化和动态脱敏策略。
