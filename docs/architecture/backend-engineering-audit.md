...
- 接口文档缺少统一 OpenAPI 信息、分组、权限说明。
- 统一错误码刚起步，需要继续扩展错误码目录。
- 缺少请求链路追踪、操作日志、登录日志。
- 文件上传需要配置化的大小、类型、路径约束。
- 列表接口暂未形成统一分页模型。
- 数据字典、系统参数、审计日志、数据权限、多租户仍待规划。
...
- 补充后端测试，覆盖登录、用户、文件上传、参数校验、TraceId、OpenAPI。
...
- 增加登录日志表：记录账号、IP、User-Agent、成功/失败、失败原因。
- 增加操作日志切面：对新增、编辑、删除、启停等管理动作做审计。
