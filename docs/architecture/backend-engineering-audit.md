...
- HRMS 已完成一期企业级设计稿，并在 v0.0.51 开始落地 Phase 1A：新增 `hr_` 基础表迁移、岗位/职级/员工基础接口、独立 HR 错误码、Swagger 分组、权限初始化和授权树挂载。
...
- HRMS 当前已进入后端基础实现阶段，后续需要继续补调岗/转正/离职、合同、花名册导入导出和 Vben 风格前端页面。
