...
- 数据字典基础 CRUD 与 HRMS 员工/合同表单接入已落地，后续需要补缓存、更多业务表单接入、字典变更审计、导入导出和多租户隔离；系统参数、文件元数据、多租户仍待规划，数据权限已完成第一阶段落地，后续需要扩展到更多业务域、自定义部门集合和字段脱敏策略。
...
- 数据变更记录：关键表变更前后差异可追踪。
- 数据权限：第一阶段已按角色支持全部数据、本部门及子部门、本部门、本人四类范围，后续扩展自定义部门集合、字段级脱敏和跨业务域复用。
