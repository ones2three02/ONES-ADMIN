...
- v0.0.137 已新增 CI 环境配置审计报告归档，Jenkins 会保存后端与前端环境变量清单、生产必填项、占位值、本地默认值和敏感项审计结论，同时不读取 ignored 本地敏感文件、不输出真实密码或 Secret。
- v0.0.138 已治理 HRMS 员工管理页前端布局稳定性、受控部门范围选择、空状态主操作和登录页品牌图标预加载，继续保持 Vben / Ant Design Vue 组件体系，不新增后端接口或数据库迁移。
- v0.0.139 已治理 HRMS 合同管理页和系统用户页左右分栏列表体验，补齐稳定表格高度、受控范围选择、加载/空状态和主操作引导，继续保持 Vben / Ant Design Vue 组件体系，不新增后端接口或数据库迁移。
- v0.0.140 已沉淀后台左右分栏列表共享布局组件，并迁移 HRMS 员工管理、合同管理和系统用户页复用统一布局规则，减少页面级重复 CSS 和后续列表页体验漂移风险。
- v0.0.141 已新增前端布局治理报告和 Jenkins 门禁，将共享分栏布局使用、VXE 表格稳定高度、旧左右分栏类回退和存量 `height: auto` WARN 清单纳入 CI 发布证据链。
- v0.0.142 已新增后台列表工作区状态条，并在 HRMS 员工管理、合同管理和系统用户页补齐当前范围、待选择状态、返回全部范围和导出等主操作引导，继续保持 Vben / Ant Design Vue 组件体系，不新增后端接口或数据库迁移。
- v0.0.143 已新增 HRMS 架构治理路线图，将开源项目对标、当前系统基线、目标架构地图、三阶段演进路线、近期治理 Backlog 和关键决策记录沉淀为长期架构治理入口，本版本不新增运行时代码、后端接口或数据库迁移。
- v0.0.144 已新增前端引导式交互治理规范，将登录页、列表检索、左右分栏、抽屉表单、批量导入和高风险操作的用户引导方式沉淀为统一验收标准，本版本不新增运行时代码、后端接口或数据库迁移。
- v0.0.145 已治理扫码和手机号认证入口的配置边界提示，移除认证子页标题 emoji，补齐登录字段自动完成、辅助入口待配置状态和登录 E2E 断言；本版本不新增后端 SSO/短信接口或数据库迁移。
- v0.0.146 已新增项目状态与问题闭环审计，并修正 README 当前能力口径，将飞书/SSO 未闭环、表格稳定高度剩余清单、Manifest 基线缺失和服务职责膨胀纳入后续治理优先级；本版本仅同步产品版本元数据，不修改业务运行逻辑，不新增后端接口或数据库迁移。
- v0.0.147 已治理 11 个存量业务 VXE 表格页面的稳定高度，将 `frontend-layout-report` 自动高度清单降为 0；本版本不新增后端接口或数据库迁移。
- v0.0.148 已将系统数据初始化改为显式开关和外部密码，新增账号/IP 登录限流、数据库失败次数原子更新、受控 Swagger/Actuator、CORS、安全响应头和 Flyway 生产安全默认值；本版本不新增数据库迁移、不连接真实中间件。
- v0.0.149 已新增飞书 OAuth Provider、Redis/内存一次性流程存储、外部身份管理员预绑定、V9 迁移、Sa-Token 统一签发和第三方登录审计；真实飞书、Redis 和 MySQL 联调仍需单独审批。
...
- 新增 Flyway 数据库迁移治理：
  ...
  - `baseline-on-migrate` 默认关闭；只有经数据库现状审计、备份和人工确认的历史非空库首次纳管时，才通过环境变量临时开启并记录基线。
...
- `main`：稳定版本、正式环境部署来源。
- Jenkins 先接 `develop` 做验证门禁，根目录 `Jenkinsfile` 已在 v0.0.129 固化当前阶段的测试、类型检查、构建和仓库扫描，在 v0.0.130 补齐版本一致性门禁，在 v0.0.131 统一为 `scripts/ci/verify.sh` 阶段入口，在 v0.0.132 增加构建元数据归档，在 v0.0.133 增加机器可读验证摘要归档，在 v0.0.134 增加接口治理报告归档，在 v0.0.135 增加数据库迁移审计报告归档，在 v0.0.136 增加发布证据包归档，并在 v0.0.137 增加环境配置审计报告归档，不包含部署动作：
  ...
  - 生成 `.ci-artifacts/environment-config-report.json` 并归档，便于从 Jenkins 构建反查环境变量清单、生产必填项、占位值、本地默认值和敏感项审计结论
  ...
  - 环境配置审计只输出变量名、分类、默认值类型和敏感标记；真实中间件地址、账号、密码仍只能放在服务器环境变量、Jenkins 凭据或 ignored 本地配置中
